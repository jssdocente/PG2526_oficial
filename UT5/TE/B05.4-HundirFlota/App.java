...
public class App {
    ...
    /**
     * Obtiene el valor de una celda adyacente en una dirección dada.
     * 
     * @param matriz Matriz donde buscar.
     * @param fila Fila actual.
     * @param col Columna actual.
     * @param dir Índice de la dirección (0=arriba, 1=derecha, 2=abajo, 3=izquierda).
     * @return El valor de la celda adyacente, o -1 si la posición se sale del tablero.
     */
    public static int getValorCelda(int[][] matriz, int fila, int col, int dir) {
        int nFila = fila + direcciones[dir][0];
        int nCol = col + direcciones[dir][1];

        // Comprobación de límites del tablero
        if (nFila >= 0 && nFila < TAM && nCol >= 0 && nCol < TAM) {
            return matriz[nFila][nCol];
        }
    ...
    private static final String[] colores = { ANSI_BLACK, ANSI_WHITE, ANSI_CYAN, ANSI_BLUE, ANSI_YELLOW, ANSI_GREEN, ANSI_PURPLE, ANSI_RED, ANSI_GREY };
