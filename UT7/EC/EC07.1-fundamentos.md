...
### Contexto
### Contexto
No todos los dispositivos se conectan de la misma manera. Un termostato necesita calibrarse, un enchufe solo necesita activar el relé. Sin embargo, **todos** tienen nombre, ubicación y estado de encendido.

Aquí es donde brillan las clases abstractas. Su objetivo es doble:
1. **Compartir código y datos**: Evitamos repetir atributos (`nombre`, `ubicacion`) y métodos comunes (`encender`, `apagar`, `reiniciar`) en cada subclase.
2. **Definir un contrato**: Obligamos a las subclases a implementar comportamientos específicos que no podemos definir de forma genérica (como `conectar`).

### Objetivo
Convertir `Dispositivo` en una clase abstracta para centralizar lo común y abstraer lo específico.
...
1. **Modifica la clase `Dispositivo`**:
   ...
   - **Nuevo**: Añade un método concreto `void reiniciar()` que reutilice la lógica común:
     ```java
     public void reiniciar() {
         System.out.println("\n--- Reiniciando " + getNombre() + " ---");
         apagar();
         encender();
         conectar(); // Llama a la implementación específica de cada hijo
     }
     ```
     > Este método demuestra la potencia de las clases abstractas: define un comportamiento común (**Template Method**) que orquesta métodos concretos y abstractos.
...
public class PruebaEjercicio2 {
    public static void main(String[] args) {
        ...
        // Probamos el método común (reutilización de código)
        dispositivos[0].reiniciar();
    }
}
```
